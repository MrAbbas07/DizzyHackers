...
interface FireIncidentTableProps {
  incidents: Incident[]
  onSelectIncident: (incident: Incident) => void
  selectedIncidentId: string | undefined
}

export function FireIncidentTable({ incidents, onSelectIncident, selectedIncidentId }: FireIncidentTableProps) {
  ...
  const handleDeployUnits = () => {
    ...
    console.log(`Deploying fire units to incident ${selectedIncidentForDeploy?.id}:`, selectedUnits)

    // Close the dialog
    setDeployDialogOpen(false)

    // Show notification (in a real app, this would be a toast)
    alert(`Fire units deployed to incident ${selectedIncidentForDeploy?.id}`)
  }
    ...
    <>
      <Table>
        ...
        <TableBody>
          {incidents.map((incident) => (
            <TableRow
              key={incident.id}
              className={selectedIncidentId === incident.id ? "bg-red-50" : ""}
            ...
            >
              ...
              <TableCell>
                <Badge
                  variant="outline"
                  className={
                    incident.hazardLevel === "High"
                      ? "border-red-500 text-red-500"
                      : incident.hazardLevel === "Medium"
                        ? "border-orange-500 text-orange-500"
      ...
      <Dialog open={deployDialogOpen} onOpenChange={setDeployDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Deploy Fire Units</DialogTitle>
            <DialogDescription>
              Select fire units to deploy to incident {selectedIncidentForDeploy?.id} at{" "}
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select defaultValue={selectedIncidentForDeploy?.hazardLevel.toLowerCase()}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              <Label>Available Fire Units</Label>
            ...
            <div className="space-y-2">
              ...
              <Select defaultValue="water">
                ...
                <SelectContent>
                  ...
                  <SelectItem value="retardant">Fire Retardant</SelectItem>
